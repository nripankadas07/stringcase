...
from stringcase._convert import (
    to_snake_case,
    to_camel_case,
    to_pascal_case,
    to_kebab_case,
...
def to_snake(text: str) -> str:
    _check(text)
    return to_snake_case(_split_words(text))


def to_camel(text: str) -> str:
    _check(text)
    return to_camel_case(_split_words(text))


def to_pascal(text: str) -> str:
    _check(text)
    return to_pascal_case(_split_words(text))


def to_kebab(text: str) -> str:
    _check(text)
    return to_kebab_case(_split_words(text))


def to_constant(text: str) -> str:
    _check(text)
    return to_constant_case(_split_words(text))


def to_dot(text: str) -> str:
    _check(text)
    return to_dot_case(_split_words(text))


def to_title(text: str) -> str:
    _check(text)
    return to_title_case(_split_words(text))


def to_path(text: str) -> str:
    _check(text)
    return to_path_case(_split_words(text))
...
def convert(text: str, style: str) -> str:
    ...
    if style not in style_map:
        raise StringcaseError(
            f"Unknown style '{style}'. Valid styles: {', '.join(style_map.keys())}"
        )
